...
const wethAddress = erc20Address.WETH;
const tokenToArbitrage = erc20Address.USDC
const pairUniswapAddress = "0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc";
const pairPancakeAddress = "0x2E8135bE71230c6B1B4045696d41C09Db0414226";
//const tokenToArbitrage = '0xA4D7EEC256B5C6e89723001A70B7db06b40e6FF0';

//4. Wallet
const flashbotsEndpoint = "https://relay.flashbots.net";
const privateKey = process.env.FORK_WALLET_SECRET;
const provider = new ethers.providers.JsonRpcProvider("http://127.0.0.1:8545/");
const wsProvider = new ethers.providers.WebSocketProvider("ws://127.0.0.1:8545");
const wallet = new ethers.Wallet(privateKey, provider);

//5. Contract Instances
const uniswapInstance = new ethers.Contract(uniswapRouterAddress, uniswapAbi, wallet);
const pancakeRouterInstance = new ethers.Contract(pancakeRouterAddress, pancakeRouterAbi, wallet);
...
const pairPancake = new ethers.Contract(pairPancakeAddress, pairAbi, wallet);
const amountToArbitrage = new ethers.utils.parseUnits("1","ether");
const bribePriorityFee = ethers.utils.parseUnits("2000","gwei");
const chainId = 1; //ETH MAINNET (HARHDAT TEST AND ALCHEMLY MAINNET)
let flashbotsProvider = null;

//8. Then Create the function checkPriceAtExchange()
const checkPriceAtExchange = async (exchangeToUse, amountToSell, a, b) => {
    if (exchangeToUse === 'uniswap') {
        return await uniswapInstance.getAmountOut(amountToSell, a, b);
    } else if (exchangeToUse === 'pancake') {
        return await pancakeRouterInstance.getAmountOut(amountToSell, a, b);
    }
}

// 12. Create the function prepareFlashbotsBundle() to prepare the bundle with the transaction
const prepareFlashbotsBundle = async (isBuyAtUniswap, buyAmount, sellAmount, block) => {
    ...
    if (isBuyAtUniswap) {
        console.log('Buy at uniswap, sell at pancakeswap');
        firstDex = uniswapInstance;
        secondDex = pancakeRouterInstance;
    } else {
        console.log('Buy at pancakeswap, sell at uniswap');
        firstDex = pancakeRouterInstance;
        secondDex = uniswapInstance;
    }
    const maxGasFee = block.baseFeePerGas.mul(2).add(bribePriorityFee);

    approvalTransaction = {
        signer: wallet,
        transaction: await tokenInstance.populateTransaction.approve(
            isBuyAtUniswap ? pancakeRouterAddress : uniswapRouterAddress,
            ethers.utils.parseUnits('9999999999999999999', 'ether'),
            {
                type: 2,
    ...
    firstBuy = {
        ...
        transaction: await firstDex.populateTransaction.swapExactETHForTokens(
            '0',
            [
                wethAddress, tokenToArbitrage
            ],
            wallet.address,
            '999999999999999999999', //Deadline
            {
                value: buyAmount,
                type: 2,
                maxFeePerGas: maxGasFee,
    ...
    secondSell = {
        ...
        transaction: await secondDex.populateTransaction.swapExactTokensForETH(
            ...
            0,
            [
                tokenToArbitrage, wethAddress
            ],
            wallet.address,
            99999999999999, // deadline
            {
                type: 2,
                maxFeePerGas: maxGasFee,
                maxPriorityFeePerGas: bribePriorityFee,
...
//14. Create the function to send to the bundle
const sendSignedBundle = async (signedBundle, blockNumber) => {
    ...
    try {
        ...
        console.log('Bundle submitted, waiting bundleHash', bundleSubmission.bundleHash);
        const waitResponse = await bundleSubmission.wait();
        console.log(`Wait Response Flashbots: ${FlashbotsBundleResolution[waitResponse]} `);
        if (
            ...
            console.log('________________________________________________');
            console.log('________________________________________________');
            console.log('________________Bundle included!________________');
            console.log('________________________________________________');
            console.log('________________________________________________');
            return true //Return As valid
        } else if (waitResponse === FlashbotsBundleResolution.AccountNonceTooHigh) {
            console.log('AccountNonceTooHigh the victims transaction has been confirmed already');
        } else {
            try {
                console.log({
                ...
                console.log('Error getting bundle and user stats', e);
            }
            console.log('bundles submitted');
        }
    } catch (e) {
        console.log('Error when checking data:', e);
    }
}

//6. Start Function With Flashbots
const start = async () => {
    console.log('User', wallet.address);
    flashbotsProvider = await FlashbotsBundleProvider.create(provider, wallet. flashbotsEndpoint);

    //7. Get Reserves Inside Start
    let reserveUniswap;
    let reservePancake;
    try {
        reserveUniswap = await pairUniswap.getReserves();
    } catch (e) {
        return console.log("Error getting pair reverves after generating the pair address")
    }
    try {
        reservePancake = await pairPancake.getReserves();
    } catch (e) {
        return console.log("Error getting pair reverves after generating the pair address")
    }
    let uniswapA;
    ...
    let pancakeA;
    let pancakeB;
    if (wethAddress < tokenToArbitrage) {
        uniswapA = reserveUniswap._reserve0;
        uniswapB = reserveUniswap._reserve1;
        pancakeA = revervePancake._reserve0;
        pancakeB = reservePancake._reserve1;
    } else {
        uniswapA = reserveUniswap._reserve1;
        uniswapB = reserveUniswap._reserve0;
        pancakeA = reservePancake._reserve1;
        pancakeB = reservePancake._reserve0;
    }

    //9. Get the amounts out for both exchanges to see which one gives you the most tokens per ether (inside start())
    const a = await checkPriceAtExchange('uniswap', amountToArbitrage, uniswapA, uniswapB);
    const b = await checkPriceAtExchange('pancake', amountToArbitrage, pancakeA, pancakeB);
    console.log('a', String(a));
    console.log('b', String(b));

    //10. Calculate the Potential profit by buying the most per giver
    let resultingETH
    let isBuyAtUniswap = a.gt(b);
    if (isBuyAtUniswap) {
        console.log('A larger than B');
        // Calculate how much ETH we get after selling A tokens on B exchange
        resultingETH = await checkPriceAtExchange('pancake', a, pancakeA, pancakeB);
    } else {
        console.log('B larger than A')
        resultingETH = await checkPriceAtExchange('uniswap', b, uniswapA, uniswapB);
    }
    const profit = resultingETH.sub(amountToArbitrage);
    console.log('ResultingETH', resultingETH.toString());
    console.log('Profit', profit.toString());

    //11. If There's no profit, simply skip this
    if (profit.gt(0)) {
        console.log('Profit is positive lets make the trade');
    } else {
        console.log('Not profitable, skip');
    }

    //13. Come Back to start() prepare the bundle
    let block = await wsProvider.getBlock();
    let bundle = null;
    if (isBuyAtUniswap) {
        bundle = await prepareFlashbotsBundle(isBuyAtUniswap, amountToArbitrage, a, block);
    } else {
        bundle = await prepareFlashbotsBundle(isBuyAtUniswap, amountToArbitrage, b, block);
    }

    //15. Sign and Send the bundle making tried until it's included
    const signedBundle = await flashbotsProvider.signBundle(bundle);
    let BundleIncluded = false;
    //Keeps repeating until the bundle is accepted
    while(!BundleIncluded) {
        block = await wsProvider.getBlock();
        bundleIncluded = await sendSignedBundle(signedBundle, block.number);
    }
    process.exit(0);
}
start();